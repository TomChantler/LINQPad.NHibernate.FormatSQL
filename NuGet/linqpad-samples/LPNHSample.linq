...
void Main()
{
	...
	// The best way to do this is to download the NuGet package from here: https://www.nuget.org/packages/System.Data.SQLite.Core/1.0.105.2, extract it 
	...
	var path = @"C:\Users\Tom\Downloads\system.data.sqlite.core.1.0.105.2\build\net45";
	System.Environment.SetEnvironmentVariable("PreLoadSQLite_BaseDirectory", path);

	using (var nHibernateSqlOutputRedirector = new NHibernateSqlOutputRedirector(true))
	{
		var authors = LINQPad.NHibernate.FormatSQL.Sample.Main.Get(true);
		authors.Dump();
	}
}